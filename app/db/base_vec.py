...
def get_last_id():
    ...
    try:
        # id 필드에서 card_숫자 형식의 최댓값 찾기
        cur.execute("""
            SELECT metadata->>'id' 
            FROM card_tbl 
            WHERE metadata->>'id' LIKE 'card_%'
            ORDER BY CAST(SUBSTRING(metadata->>'id' FROM 'card_([0-9]+)') AS INTEGER) DESC
            LIMIT 1;
        """)
        result = cur.fetchone()
...
def embed_and_save(chunks):
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            cur.execute(
                "INSERT INTO card_tbl (content, metadata, embedding) VALUES (%s, %s, %s)",
